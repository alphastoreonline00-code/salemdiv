...
async function enviarDivulgacao(id, texto, sock) {
    await simularDigitacao(id, sock, DELAY_DIGITANDO); await delay(DELAY_PULADOS);
    if (midiaAtiva) {
        if (fs.existsSync('./Post.mp4')) return await sock.sendMessage(id, { video: fs.readFileSync('./Post.mp4'), caption: texto });
        if (fs.existsSync('./Post.jpg')) return await sock.sendMessage(id, { image: fs.readFileSync('./Post.jpg'), caption: texto });
        if (fs.existsSync('./Post.png')) return await sock.sendMessage(id, { image: fs.readFileSync('./Post.png'), caption: texto });
    }
    return await sock.sendMessage(id, { text: texto, linkPreview: true });
}
...
await enviarDivulgacao(g.id, msgTexto, sockAtual);
updateStat('divulgou', '++'); sucessos++;
addLog(`[${p}] Divulgou + ${nome}`);
